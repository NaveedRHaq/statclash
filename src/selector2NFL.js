...
const GameButtons2 = ({ onRevealWinners, updateScore }) => {
  ...
  const allGamesSelected = Object.keys(selectedGames).length === rounds.firstRound.length;
  ...
  const handleRevealWinners = () => {
    ...
    const winners = rounds.firstRound.map((game) => ({
      match: game.name,
      winner: game.winner,
    }));
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        {nflRounds.secondRound.map((game) => (
