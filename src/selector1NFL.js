...
import { nflRounds, nflTeamColors } from './nflData';

const GameButtons1 = ({ onRevealWinners, updateScore }) => {
  ...
  const allGamesSelected = Object.keys(selectedGames).length === nflRounds.firstRound.length;
  ...
  const handleRevealWinners = () => {
    ...
    const winners = nflRounds.firstRound.map((game) => ({
      match: game.name,
      winner: game.winner,
    }));
            ...
            {game.participants.map((participant) => (
              <button
                ...
                style={{
                  margin: '3px',
                  padding: '10px 20px',
                  backgroundColor: nflTeamColors[participant.name].backgroundColor,
                  color: nflTeamColors[participant.name].color,
                  border: 'none',
                  borderRadius: '4px',
                  fontSize: '16px',
                  cursor: 'pointer',
